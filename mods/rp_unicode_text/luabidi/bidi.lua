...
-- @license MIT
-- @module bidi
local ucdn = dofile(minetest.get_modpath(minetest.get_current_modname()).."/luabidi/bidi/ucdn.lua")
local bracket = dofile(minetest.get_modpath(minetest.get_current_modname()).."/luabidi/bidi/bracket.lua")
...
local function typeForLevel(level)
  if bit.band(level, 1) == 0 then return L else return R end
end
...
function Paragraph:determineExplicitEmbeddingLevels()
  ...
  for i = 1, self:length() do
    ...
    if t == RLE or
      ...
      if isRTL then
        -- least greater odd
        newLevel = bit.bor(stack:lastEmbeddingLevel() + 1, 1)
      else
        -- least greater even
        newLevel = bit.band(stack:lastEmbeddingLevel() + 2, bit.bnot(1))
      end
...
function IsolatingRunSequence:resolveImplicitLevels()
  ...
  if bit.band(self.level, 1) == 0 then -- even level
    for i = 1, #self.types do
      local t = self.types[i]
      -- Rule I1
      if t == L then
...
local function computeReordering(levels)
  ...
  for i = 1, lineLength do
    ...
    if bit.band(level, 1) ~= 0 and level < lowestOddLevel then lowestOddLevel = level end
  end
